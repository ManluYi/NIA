#!/usr/bin/env python3
"""Summarize OptiMathSAT result files into a TSV."""
...
UNARY_VALUE_RE = re.compile(r"^\(\s*([+-])\s+(.+?)\s*\)$")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser()
    parser.add_argument("result_dir", nargs="?", default="results/optimathsat")
    parser.add_argument("output_tsv", nargs="?", default="results/optimathsat_summary.tsv")
...
def normalize_objective_value(value: str) -> str:
    value = value.strip()
    match = UNARY_VALUE_RE.match(value)
    if match:
...
def parse_result_file(path: Path, input_dir: str) -> tuple[str, str, str, str]:
    ...
    if objective_value:
        objective_value = normalize_objective_value(objective_value)
